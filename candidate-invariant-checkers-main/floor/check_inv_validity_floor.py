...
class SMTLIBTranslator:
    ...
    def _generate_smtlib(self, translated_expr):
        ...
        # return f is an integer f <= x < f + 1.0
        return f"""{declarations}
(assert (or (fp.isNaN Double_Variable_Orig_x) (fp.leq Double_Variable_return Double_Variable_Orig_x)))
(assert (or (fp.isNaN Double_Variable_Orig_x) (fp.lt Double_Variable_Orig_x (fp.add RTN Double_Variable_return one_fp))))
(assert (or (not (fp.isNaN Double_Variable_Orig_x)) (fp.isNaN Double_Variable_return)))
(assert (not {translated_expr_up}))
(check-sat)"""
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        sys.stderr.write("Usage: python check_inv_validity_floor.py \"formula\"\n")
        sys.stderr.write("Example: python check_inv_validity_floor.py \"(Double_Variable_0 = 1)\"\n")
        sys.exit(1)
