...
class SMTLIBTranslator:
    ...
    def _collect_variables(self, expr):
        ...
        self.variables.add('Integer_Variable_Orig_element')
        self.variables.add('Integer_Set_Variable_Orig_intSet')
        self.variables.update(re.findall(r'Integer_Variable_[a-zA-Z0-9_]+', expr))
        self.variables.update(re.findall(r'Integer_Set_Variable_[a-zA-Z0-9_]+', expr))
    ...
    def _parse_expression(self, expr):
        ...
        for op in ["=", "!=", ">", "<", ">=", "<=","in"]:
            ...
            if parts:
                ...
                if op == "in":
                    return f"({smt_op} {right} (seq.unit {left}))"
                return f"({smt_op} {left} {right})"
        ...
        if re.fullmatch(r'Integer_Variable_[a-zA-Z0-9_]+', expr):
            return expr
        if re.fullmatch(r'Integer_Set_Variable_[a-zA-Z0-9_]+', expr):
            return expr
        
        raise ValueError(f"Could not parse expressionnnn: {expr}")
    ...
    def _generate_smtlib(self, translated_exprs, ground_truth_expr):
        ...
        declarations += "\n" + "\n".join(
            f"(declare-const {var} (Seq Int))"
            for var in sorted(self.variables) if var.startswith("Integer_Set_Variable_")
        )
    ...
    def process_invariant_line(line):
        ...
        for i, var in enumerate(variables):
            ...
            if "intSet" in var:
                var_mapping['Integer_Set_Variable_0'] = "Integer_Set_Variable_Orig_intSet"
            else:
                var_mapping[f'Integer_Variable_0'] = var_name
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        sys.stderr.write("Usage: python check_inv_validity_getmin.py \"formula\"\n")
        sys.stderr.write("Example: python check_inv_validity_getmin.py \"(Integer_Variable_0 = 1)\"\n")
        sys.exit(1)
    ...
    try:
        ground_truth = "(seq.contains Integer_Set_Variable_Orig_intSet (seq.unit Integer_Variable_Orig_element))"
