...
-- This checker is for the push method
one sig Null { }
sig Elem { }
sig StackAr {
...
-- Fact regarding precondition of push, there's enough space for pushing:
fact {
	SK.s.top < SK.s.size - 1
}
...
-- top is increased by one after push
pred groundTruth2[] {
    SK.s1.top = add[SK.s.top,1]
}

-- element added at position top in array after push
pred groundTruth3[] {
    SK.s1.arr[SK.s.top] = SK.x
}

-- no existing element is modified
pred groundTruth4[] {
