...
-- This checker is for the pop method
one sig Null { }
sig Elem { }
sig StackAr {
...
-- Fact regarding precondition of pop, stack is not empty:
fact {
	SK.s.top >= 0
}
...
-- top is reduced by one after pop
pred groundTruth2[] {
    SK.s1.top = sub[SK.s.top,1]
}

-- previous top has been set to null after pop
pred groundTruth3[] {
    SK.s1.arr[SK.s.top] = Null
}

-- elements below previous top are unchanged after pop
pred groundTruth4[] {
