...
def translate_assertion(assertion_line, predicate_counter):
    ...
    if match:
        ...
        if len(variables) >= 2:
            ...
            if len(var_map) >= 2:  # Need at least 2 variables for XOR
                ...
                default_stack = "SK.s1" if has_orig else "SK.s1"
    ...
    # Pattern : assertion_line is not a FuzzedInvariant
...
def translate_single_field(field):
    ...
    if field == 'this':
        return 'SK.s1'
    elif field == 'this.topOfStack':
        return 'SK.s1.top'
    elif field == '#(StackAr.theArray)':
        return 'SK.s1.size'
    else:
        return None
