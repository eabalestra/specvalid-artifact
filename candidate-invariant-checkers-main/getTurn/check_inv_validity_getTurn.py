...
class SMTLIBTranslator:
    ...
    def _generate_smtlib(self, translated_expr):
        ...
        # res = 1 or res = -1 or res = 0
        # sin(a2 - a1) > 0 implies res = 1
        # sin(a2 - a1) < 0 implies res = -1
        # sin(a2 - a1) = 0 implies result = 0
        return f"""{declarations}
(assert (forall ((x Real)) (and (<= -1 (f_sin x)) (<= (f_sin x) 1))))
(assert (or (= Integer_Variable_return 1) (or (= Integer_Variable_return (- 1)) (= Integer_Variable_return 0))))
(assert (or (not (> (f_sin (- Double_Variable_Orig_ang2 Double_Variable_Orig_ang1)) 0)) (= Integer_Variable_return 1)))
(assert (or (not (< (f_sin (- Double_Variable_Orig_ang2 Double_Variable_Orig_ang1)) 0)) (= Integer_Variable_return (- 1))))
(assert (or (not (= (f_sin (- Double_Variable_Orig_ang2 Double_Variable_Orig_ang1)) 0)) (= Integer_Variable_return 0)))
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        sys.stderr.write("Usage: python check_inv_validity_getTurn.py \"formula\"\n")
        sys.stderr.write("Example: python check_inv_validity_getTurn.py \"(Double_Variable_0 = 1)\"\n")
        sys.exit(1)
