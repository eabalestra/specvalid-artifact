...
def translate_assertion(assertion_line, predicate_counter):
    ...
    match = re.match(
        r'daikon\.Quant\.eltsEqual\((this\.[A-Za-z0-9_]+),\s*null\)',
        assertion_line
    )
    if match:
        print("Pattern 20")
...
def translate_single_field(field):
    """Translate a single field reference for MaxBag.getMax"""
    field = field.strip()
    ...
    if 'size(this._var66253[])' in field:
        if '-1' in field:
            if 'orig(' in field:
                return 'sub[#(SK.s._var66253),1]'
            else:
                return 'sub[#(SK.s1._var66253),1]'
        else:
            if 'orig(' in field:
                return '#(SK.s._var66253)'
            else:
                return '#(SK.s1._var66253)'

    if field == 'this._var49':
        return 'SK.s1._var49'

    if field == 'this._var50':
        return 'SK.s1._var50'

    # size(this._var49[])
    if 'size(this._var49[])' in field:
        if '-1' in field:
            if 'orig(' in field:
                return 'sub[#(SK.s._var49),1]'
            else:
                return 'sub[#(SK.s1._var49),1]'
        else:
            if 'orig(' in field:
                return '#(SK.s._var49)'
            else:
    ...
    # size(this._var50[])
    if 'size(this._var50[])' in field:
        if '-1' in field:
            if 'orig(' in field:
                return 'sub[#(SK.s._var50),1]'
            else:
                return 'sub[#(SK.s1._var50),1]'
        else:
            if 'orig(' in field:
                return '#(SK.s._var50)'
            else:
    ...
    if field.startswith('orig('):
        ...
        if inner == 'this':
            return 'SK.s'
        elif inner == 'this._var49':
            return 'SK.s._var49'
        elif inner == 'this._var50':
            return 'SK.s._var50'
...
def translate_orig_reference(inner):
    """Translate orig(...) references (used in logical expressions)"""
