...
def translate_assertion(assertion_line, predicate_counter):
    ...
    if match:
        ...
        if len(variables) >= 2:
            ...
            if len(var_map) >= 2:  # Need at least 2 variables for XOR
                ...
                default_queue = "SK.q" if has_orig else "SK.q1"
                
                left_translated = left_translated.replace('this.', f'{default_queue}.')
                right_translated = right_translated.replace('this.', f'{default_queue}.')
    ...
    # Pattern 15: assertion_line is not a FuzzedInvariant
    if "FuzzedInvariant" not in assertion_line:
        # matches: this.currentSize == \old(this.currentSize)
        match = re.match(r'(.+)\s*([!=<>]=)\s*(.+)', assertion_line)
        if match:
            left_field = match.group(1).strip()
            operator = match.group(2)
            right_field = match.group(3).strip()
            left_translated = translate_single_field(left_field)
            print(left_field, "->" , left_translated)
            right_translated = translate_single_field(right_field)
            print(right_field, "->" , right_translated)
...
def translate_single_field(field):
    ...
    # check if field matches int
    if re.match(r'^-?\d+$', field):
        return field

    # Handle size patterns
    if 'size(this.theArray[])' in field:
        if '-1' in field:
            if 'orig(' in field:
                return 'sub[SK.q.size,1]'
            else:
                return 'sub[SK.q1.size,1]'
        else:
            if 'orig(' in field:
                return 'SK.q.size'
            else:
    ...
    if field.startswith('orig('):
        ...
        if inner == 'this.currentSize':
            return 'SK.q.currentSize'
        if inner == 'this.front':
            return 'SK.q.front'
        if inner == 'this.back':
            return 'SK.q.back'
    ...
    if field.startswith('\old('):
        print('translating old field:', field)
        inner = field[5:-1]  # Remove 'old(' and ')'
        if inner == 'this.currentSize':
            return 'SK.q.currentSize'
        if inner == 'this.front':
            return 'SK.q.front'
    ...
    if field == 'this':
        return 'SK.q1'
    elif field == 'this.currentSize':
        return 'SK.q1.currentSize'
    elif field == 'this.front':
        return 'SK.q1.front'
    elif field == 'this.back':
        return 'SK.q1.back'
    elif field == '#(QueueAr.theArray)':
        return 'SK.q1.size'
    else:
        return None
...
def translate_orig_reference(inner):
    ...
    if inner == 'this':
        return 'SK.q'
    elif inner == 'this.currentSize':
        return 'SK.q.currentSize'
    elif inner == 'this.front':
        return 'SK.q.front'
    elif inner == 'this.back':
        return 'SK.q.back'
    elif inner == 'this.theArray':
        return 'SK.q.arr'
    elif 'size(this.theArray[])' in inner:
        if '-1' in inner:
            return 'sub[SK.q.size,1]'
        else:
            return 'SK.q.size'
...
def main():
    ...
    if generated:
        with open('generated-preds.txt', 'w') as f:
            ...
            f.write("// Checking against ground truth predicates: groundTruth1, groundTruth2, groundTruth3, groundTruth4, groundTruth5 \n\n")
