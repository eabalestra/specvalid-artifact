...
-- This checker is for the enqueue method
one sig Null { }
sig Elem { }
sig QueueAr {
...
-- front dind't change after enqueue.
pred groundTruth2[] {
    SK.q1.front = SK.q.front
}
-- currentSize is increased after enqueue.
pred groundTruth3[] {
    SK.q1.currentSize = add[SK.q.currentSize,1]
}
-- back is increased or 0 after enqueue.
pred groundTruth4[] {
    --(add[SK.q.back, 1] = SK.q.size implies SK.q1.back = 0) and (add[SK.q.back, 1] < SK.q.size implies SK.q1.back = add[SK.q.back, 1])
    SK.q1.back = 0 or SK.q1.back = add[SK.q.back, 1]
}
-- array contains element x at back after enqueue.
pred groundTruth5[] {
    some SK.q1.arr[SK.q1.back]
}
