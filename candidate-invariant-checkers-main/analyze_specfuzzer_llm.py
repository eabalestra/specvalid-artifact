...
specfuzzer_llm_results_dir = os.path.join(curr_dir, "../specvalid-results")
target_llm = sys.argv[1]  # e.g., "model_GPT51", "model_NDeepSeekR1", "model_NLlama3370Instruct"
invs_type = sys.argv[2]  # e.g., "all_invs", "buckets"
single_subject = sys.argv[3] if len(sys.argv) > 3 else None

if invs_type not in ["all_invs", "buckets"]:
    print("Invalid invs_type. Use 'all_invs' or 'buckets'.")
    sys.exit(1)

def get_invs_file(folder_path, invs_t, llm):
    output_file_ending = "-specvalid.assertions"
    if invs_t == "all_invs":
        output_folder = os.path.join(folder_path, llm)
    else:
        output_folder = os.path.join(folder_path, f"bucketing/{llm}")
    if not os.path.exists(output_folder):
        # use specfuzzer invs
        output_folder = output_folder.replace("specvalid-results", "specfuzzer-subject-results")
        output_folder = output_folder.replace(llm, "output")
        output_folder = output_folder.replace("SimpleMethods_", "simple-examples_")
        output_folder = output_folder.replace("ArithmeticUtilsNew_", "ArithmeticUtils_")
        output_file_ending = "-1.assertions"
        if not os.path.exists(output_folder):
            return None
    for file in os.listdir(output_folder):
        if invs_t == "all_invs" and file.endswith(output_file_ending):
...
for folder in os.listdir(specfuzzer_llm_results_dir):
    folder_path = os.path.join(specfuzzer_llm_results_dir, folder)
    if os.path.isdir(folder_path):
        invs_file = get_invs_file(folder_path, invs_type, target_llm)
        if invs_file is None:
            continue

        if "ListComp02" in folder:
            folder = folder.replace("ListComp02", "listcomp02")
        if "RingBuffer" in folder:
            folder = folder.replace("RingBuffer", "ringbuffer")
        if "Map_" in folder:
            folder = folder.replace("Map_", "map_")
        if "MaxBag" in folder:
            folder = folder.replace("MaxBag", "maxbag")
        if "SimpleMethods" in folder:
            folder = folder.replace("SimpleMethods", "simple-examples")
...
# Write CSV file
output_csv = f"specfuzzer-{target_llm}-results-{invs_type}.csv"
