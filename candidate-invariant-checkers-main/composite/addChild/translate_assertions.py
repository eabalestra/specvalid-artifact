...
def translate_assertion(assertion_line, predicate_counter):
    ...
    if "ancestors" in assertion_line:
        print("Skipping assertion with 'ancestors'")
        return None, predicate_counter
    if "size(" in assertion_line:
        print("Skipping assertion with 'size('")
        return None, predicate_counter
    ...
    if match:
        ...
        if field.startswith("orig(") and field.endswith(")"):
            field_translated = translate_orig_reference(field[5:-1])
        else:
    ...
    if match:
        ...
        if len(variables) == 2:
            if variables[0].startswith("orig(") and variables[0].endswith(")"):
                var1 = translate_orig_reference(variables[0][5:-1])
            else:
                var1 = translate_single_field(variables[0])
            if variables[1].startswith("orig(") and variables[1].endswith(")"):
                var2 = translate_orig_reference(variables[1][5:-1])
            else:
    ...
    if match:
        ...
        if len(variables) == 3:
            if variables[0].startswith("orig(") and variables[0].endswith(")"):
                var0 = translate_orig_reference(variables[0][5:-1])
            else:
                var0 = translate_single_field(variables[0])
            if variables[1].startswith("orig(") and variables[1].endswith(")"):
                var1 = translate_orig_reference(variables[1][5:-1])
            else:
                var1 = translate_single_field(variables[1])
            if variables[2].startswith("orig(") and variables[2].endswith(")"):
                var2 = translate_orig_reference(variables[2][5:-1])
            else:
    ...
    if match:
        ...
        alloy_field_2 = translate_singled_old_field(field2)

        if alloy_field_1 is None or alloy_field_2 is None:
            return None, predicate_counter
    ...
    # Pattern 17.2: field1.nav1.nav2.... == field2.nav2.nav3...
    ...
    if match:
        ...
        alloy_field_2 = alloy_field_2.replace('null', "Null")
    ...
    if match:
        ...
        if not nav1:
            nav1 = ""
        alloy_field_1 = translate_single_field(field1+nav1)
        #nav1 = nav1.replace('parent', 'SK.post_parent').replace('children', 'SK.post_children')


        if alloy_field_1 is None:
            return None, predicate_counter
        pred_name = f"fuzzedInv{predicate_counter}"
        pred_body = f"""pred {pred_name}[] {{
        some {alloy_field_1}
    ...
    if match:
        ...
        field = field.replace('orig(this).', '(SK.pre_this).')
    ...
    if match:
        ...
        nav = nav.replace('parent', 'SK.pre_parent').replace('max_child', 'SK.pre_max_child').replace('children', 'SK.pre_children')
        opr2 = opr2.replace('children', '(SK.pre_children)').replace('parent', '(SK.pre_parent)')
        opr2 = opr2.replace('max_child', '(SK.pre_max_child)')
        if 'init_value' in opr2:
            opr2 = opr2.replace('init_value', '(SK.pre_init_value)')
        else:
            opr2 = opr2.replace('value', '(SK.pre_value)')
    ...
    if match:
        ...
        nav = nav.replace('parent', 'SK.post_parent').replace('max_child', 'SK.post_max_child').replace('children', 'SK.post_children')
        opr1 = opr1.replace('children', '(SK.post_children)').replace('parent', '(SK.post_parent)')
        opr1 = opr1.replace('max_child', '(SK.post_max_child)')
        if 'init_value' in opr1:
            opr1 = opr1.replace('init_value', '(SK.post_init_value)')
        else:
            opr1 = opr1.replace('value', '(SK.post_value)')
        opr2 = opr2.replace('children', '(SK.post_children)').replace('parent', '(SK.post_parent)')
        opr2 = opr2.replace('max_child', '(SK.post_max_child)')
        if 'init_value' in opr2:
            opr2 = opr2.replace('init_value', '(SK.post_init_value)')
        else:
            opr2 = opr2.replace('value', '(SK.post_value)')
    ...
    if match:
        ...
        nav1 = nav1.replace('parent', 'SK.post_parent').replace('max_child', 'SK.post_max_child').replace('children','SK.post_children')
        nav2 = nav2.replace('children', '(SK.post_children)').replace('parent', '(SK.post_parent)')
        nav2 = nav2.replace('max_child', '(SK.post_max_child)')
        nav3 = nav3.replace('children', '(SK.post_children)').replace('parent', '(SK.post_parent)')
        nav3 = nav3.replace('max_child', '(SK.post_max_child)')
    ...
    if match:
        ...
        nav1 = nav1.replace('parent', 'SK.post_parent').replace('max_child', 'SK.post_max_child').replace('children',
                                                                                                        'SK.post_children')
        nav2 = nav2.replace('children', '(SK.post_children)').replace('parent', '(SK.post_parent)')
        nav2 = nav2.replace('max_child', '(SK.post_max_child)')
        if 'init_value' in nav3:
            nav2 = nav2.replace('init_value', '(SK.post_init_value)')
        else:
            nav2 = nav2.replace('value', 'SK.post_value')
        nav3 = nav3.replace('children', '(SK.post_children)').replace('parent', '(SK.post_parent)')
        nav3 = nav3.replace('max_child', '(SK.post_max_child)')
        if 'init_value' in nav3:
            nav3 = nav3.replace('init_value', '(SK.post_init_value)')
        else:
            nav3 = nav3.replace('value', 'SK.post_value')

        pred_name = f"fuzzedInv{predicate_counter}"
        pred_body = f"""pred {pred_name}[] {{{quant} n : {alloy_field}.{nav1} | {op} ((n{nav2}) & (n{nav3}))
    }}"""
        print(pred_body)
...
def translate_single_field(field):
    ...
    if field == "c":
        return "SK.child_to_add"

    field_out =  field.replace('this.', '(SK.post_this).').replace('c.', '(SK.child_to_add).').replace('children', '(SK.post_children)').replace('parent', '(SK.post_parent)').replace('max_child', '(SK.post_max_child)')
    if "init_value" in field_out:
        return field_out.replace('init_value', '(SK.post_init_value)')
    else:
        return field_out.replace('value', '(SK.post_value)')


def translate_singled_old_field(field):
    ...
    field_out = field.replace('this.', '(SK.pre_this).').replace('c.', '(SK.child_to_add).').replace('children', '(SK.pre_children)').replace('parent', '(SK.pre_parent)').replace('max_child', '(SK.pre_max_child)')
    if "init_value" in field_out:
        return field_out.replace('init_value', '(SK.pre_init_value)')
    else:
        return field_out.replace('value', '(SK.pre_value)')
...
def translate_orig_reference(inner):
    ...
    out = inner.replace('this.', '(SK.pre_this).').replace('c.', '(SK.child_to_add).').replace('children', '(SK.pre_children)').replace('parent', '(SK.pre_parent)').replace('max_child', '(SK.pre_max_child)')
    if "init_value" in out:
        return out.replace('init_value', '(SK.pre_init_value)')
    else:
        return out.replace('value', '(SK.pre_value)')
...
def main():
    ...
    for line in input_lines:
        ...
        comment_with_assertion_line = f"-- {line}"
        if translation:
            generated.append(f"{comment_with_assertion_line}\n"+translation)
        else:
