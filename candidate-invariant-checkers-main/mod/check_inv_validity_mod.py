...
class SMTLIBTranslator:
    ...
    def _generate_smtlib(self, translated_expr):
        ...
        # result = x % m or result = x % m + m
        return f"""{declarations}
(assert (or (= Integer_Variable_return (rem Integer_Variable_Orig_x Integer_Variable_Orig_m)) (= Integer_Variable_return (+ (rem Integer_Variable_Orig_x Integer_Variable_Orig_m) Integer_Variable_Orig_m))))
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        sys.stderr.write("Usage: python check_inv_validity_mod.py \"formula\"\n")
        sys.stderr.write("Example: python check_inv_validity_mod.py \"(Integer_Variable_0 = 1)\"\n")
        sys.exit(1)
