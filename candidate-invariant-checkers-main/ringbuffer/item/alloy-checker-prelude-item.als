...
-- This checker is for the item method.
open util/integer
...
-- item does not modify state fields
pred groundTruth1[] {
    SK.rb1.start = SK.rb.start
    and SK.rb1.free = SK.rb.free
    and SK.rb1.capacity_ = SK.rb.capacity_
    and SK.rb1.size = SK.rb.size
}

-- result is the first element of the buffer
pred groundTruth2[] {
    not isEmpty[SK.rb] implies SK.result = SK.rb1.data[SK.rb1.start]
}

-- data contents stay unchanged
pred groundTruth3[] {
    all i: Int | i >= 0 and i < SK.rb.size implies SK.rb1.data[i] = SK.rb.data[i]
}
