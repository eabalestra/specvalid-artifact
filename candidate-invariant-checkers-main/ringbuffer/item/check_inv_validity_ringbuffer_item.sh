...
rm -f alloy-inv-checker-item.als
cat alloy-checker-prelude-item.als generated-preds.txt >alloy-inv-checker-item.als

echo "Checking validity of candidate invariants using Alloy..."
java -jar ../../alloy/AlloyCommandline.jar alloy-inv-checker-item.als 2>&1
total_candidates=$(grep "run" alloy-inv-checker-item.als | wc -l | awk '{print $1}')
