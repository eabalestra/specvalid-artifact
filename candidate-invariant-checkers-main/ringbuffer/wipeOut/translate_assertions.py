...
def translate_assertion(assertion_line, predicate_counter):
    ...
    # Pattern 17.1: this.field == orig(this.otherField)
    match = re.match(
        r"(this\.[A-Za-z0-9_]+)\s*==\s*orig\((this\.[A-Za-z0-9_]+)\)$",
        assertion_line,
    )
    if match:
        left_field = match.group(1)
        right_field = match.group(2)
        pre = translate_orig_reference(right_field)
        post = translate_single_field(left_field)
        if pre and post:
            pred_name = f"fuzzedInv{predicate_counter}"
            pred_body = f"pred {pred_name}[] {{\n    {post} = {pre}\n}}"
            check_pred = create_check_predicate(pred_name)
