...
-- This checker is for the count method.
open util/integer
...
-- state does not change for count
pred groundTruth1[] { 
    SK.rb = SK.rb1
}

-- if free > start then result = free - start
pred groundTruth2[] { 
    (SK.rb1.free > SK.rb1.start) 
    implies 
    SK.result = sub[SK.rb1.free, SK.rb1.start]
}

-- if free <= start then result = (size - 1) - start + free
pred groundTruth3[] { 
    (SK.rb1.free <= SK.rb1.start)
    implies
    SK.result = add[sub[SK.rb1.size, 1], sub[SK.rb1.free, SK.rb1.start]]
}
