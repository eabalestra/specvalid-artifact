...
class SMTLIBTranslator:
    ...
    def _collect_variables(self, expr):
        ...
        self.variables.add('Integer_Variable_Orig_y')
        self.variables.add('Integer_Variable_y')
        self.variables.add('Integer_Variable_Orig_var62')
        self.variables.add('Integer_Variable_var62')
    ...
    def _parse_expression(self, expr):
        ...
        if expr in {"0", "1", "-1", "0.0", "1.0", "-1.0"}:
            return expr
        if expr in {"var62"}:
            return "Integer_Variable_var62"
        if expr in {"Orig_var62"}:
            return "Integer_Variable_Orig_var62"
        if expr in {"Orig_y"}:
            return "Integer_Variable_Orig_y"
    ...
    def _generate_smtlib(self, translated_exprs, ground_truth_expr):
        ...
        declarations += "\n(assert (= Integer_Variable_var62 Integer_Variable_Orig_var62))\n"
        declarations += "\n(assert (= Integer_Variable_y Integer_Variable_Orig_y))\n"
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        sys.stderr.write("Usage: python check_inv_validity_setX.py \"formula\"\n")
        sys.stderr.write("Example: python check_inv_validity_setX.py \"(Integer_Variable_0 = 1)\"\n")
        sys.exit(1)
    ...
    try:
        ground_truth = "(= Integer_Variable_var62 (+ Integer_Variable_y 1))"
